namespace :assets do
  ...
  task :precompile_assets, [:external_assets_paths] => :environment do |t, args|
    cmd = "RAILS_ENV=#{Rails.env} EXTERNAL_ASSETS_PATHS=#{args[:external_assets_paths]} rake assets:precompile"
    puts "Running: #{cmd}"
    exec(cmd)
  end
end
