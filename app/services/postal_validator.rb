class PostalValidator
  ...
  ZIPCODES_REGEX = {
      ...
      US: /\A\d{5}([ \-]\d{4})?\z/,
      CA: /\A[ABCEGHJKLMNPRSTVXY]\d[ABCEGHJ-NPRSTV-Z][ ]?\d[ABCEGHJ-NPRSTV-Z]\d\z/i,
      DE: /\A\d{5}\z/,
      AU: /\A\d{4}\z/,
      IT: /\A\d{5}\z/,
      CH: /\A\d{4}\z/,
      ...
      DZ: /\A\d{5}\z/,
      AD: /\AAD\d{3}\z/i,
      AR: /\A([A-HJ-NP-Z]{1}\d{4}[A-Z]{3}|[a-z]{1}\d{4}[a-hj-tp-z]{3})\z/i,
      ...
      DO: /\A\d{5}\z/,
      EC: /\A([A-Z]\d{4}[A-Z]|(?:[A-Z]{2})?\d{6})?\z/i,
      EE: /\A\d{5}\z/,
      FO: /\A\d{3}\z/,
      GE: /\A\d{4}\z/,
      GL: /\A39\d{2}\z/,
      ...
      TT: /\A\d{6}\z/,
      TW: /\A\d{3}\z/,
      UK: /\A([A-PR-UWYZ0-9][A-HK-Y0-9][AEHMNPRTVXY0-9]?[ABEHMNPRVWXY0-9]? {1,2}[0-9][ABD-HJLN-UW-Z]{2}|GIR 0AA)\z/i,
      UG: /\A([a-zA-Z\d\s]){3,}\z/,
      UM: /\A([a-zA-Z\d\s]){3,}\z/,
      UY: /\A([a-zA-Z\d\s]){3,}\z/,
      VC: /\A(VC)\d{4}\z/i,
  ...
  def self.valid?(postal_code, country_code: nil)
    if ZIPCODES_REGEX.key? country_code
      ...
      return mass_validate(postal_code)
    end
  end

  private
  def self.mass_validate(postal_code)
    ZIPCODES_REGEX.each_pair do |country_code, expression|
      if expression =~ postal_code
        return true
      end
    end
    false
