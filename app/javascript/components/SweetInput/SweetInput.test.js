...
it('contains a label', () => {
  const wrapper = shallow(<SweetInput name="testInput" label="TEST LABEL" />);
  expect(wrapper.containsMatchingElement(<label>TEST LABEL</label>)).toEqual(true);
});
...
it('accepts custom input types', () => {
  const telWrapper = shallow(<SweetInput type="tel" name="testInput" label="TEST LABEL" />);
  const emailWrapper = shallow(<SweetInput type="email" name="testInput" label="TEST LABEL" />);
...
it('floats the label when the input is focused', () => {
  ...
  expect(wrapper.find('.sweet-placeholder__label').hasClass('sweet-placeholder__label--active')).toBeTruthy();
});

it('floats the label when the input has a value', () => {
  const wrapper = mount(<SweetInput value="TEST VALUE" name="testInput" label="TEST LABEL" />);
  expect(wrapper.find('.sweet-placeholder__label').hasClass('sweet-placeholder__label--full')).toBeTruthy();
});

it('accepts an `onChange` callback prop', () => {
  const fn = jest.fn();
  const wrapper = mount(<SweetInput name="testInput" label="TEST LABEL" onChange={fn} />);
