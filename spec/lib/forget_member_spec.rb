...
describe ForgetMember do
  ...
  context 'failure' do
    it 'rolls back if a step is unsuccessful' do
      allow(member).to receive(:authentication).and_throw(:error)
      catch :error do
        ForgetMember.forget(member)
      end
      expect(member.reload.attributes).to match hash_including(member_attributes.stringify_keys)
    end
  end
end
